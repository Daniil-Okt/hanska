...
class PopupManager extends Popup {
  ...
  togglePopup({ target }) {
    ...
    if (targetDataTypeElement && !target.closest('[data-popup]')) {
      this.handleVideoOpen(targetDataTypeElement);
    ...
    if (targetCloseElement) {
      ...
      if (popupToClose) {
        this.handleVideoClose(popupToClose);
  ...
  handleVideoOpen(triggerElement) {
    if (!triggerElement.classList.contains('video-popup')) return;
    const videoSrc = triggerElement.dataset.video;
    if (!videoSrc) return;

    const popupVideo = document.querySelector('[data-popup="popup-video"]');
    if (!popupVideo) return;

    const videoElement = popupVideo.querySelector('.popup-video__video');
  ...
  handleVideoClose(popup, times) {
    if (!popup.classList.contains('popup-video')) return;

    const videoElement = popup.querySelector('.popup-video__video');
    if (!videoElement) return;
